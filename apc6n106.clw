   ...
   INCLUDE('ABWINDOW.INC'),ONCE
                     ...
                     MAP
                       INCLUDE('APC6N106.INC'),ONCE        !Local module procedure declarations
                       ...
                       INCLUDE('APC6N187.INC'),ONCE        !Req'd for module callout resolution
                     END


Trig_BrowseReturRanapNewPerNota PROCEDURE                  ! Generated from procedure template - Window
   ...
   'C(0)@n-15.2@60L|M~Camp~C@n2@60L|M~N 0 tran~C@s15@64D|M~total dtg~C@N-16.2@'),FROM(Queue:Browse)
                       ...
                       SHEET,AT(4,4,445,102),USE(?CurrentTab)
                         TAB('No. Nota'),USE(?Tab:3)
                           BUTTON('Cetak &Detail'),AT(12,77,61,26),USE(?Print),LEFT,FONT('Times New Roman',8,COLOR:Black,FONT:bold),MSG('Cetak SBBM'),TIP('Cetak SBBM'),ICON(ICON:Print)
...
ReturnValue          BYTE,AUTO
  ...
  CODE
  GlobalErrors.SetProcedureName('Trig_BrowseReturRanapNewPerNota')
  ...
  INIMgr.Fetch('Trig_BrowseReturRanapNewPerNota',QuickWindow) ! Restore window settings from non-volatile store
...
ReturnValue          BYTE,AUTO
  ...
  END
  IF SELF.Opened
    INIMgr.Update('Trig_BrowseReturRanapNewPerNota',QuickWindow) ! Save window data to non-volatile store
  END
...
ReturnValue          BYTE,AUTO
  ...
  ELSE
    ...
    EXECUTE Number
      Trig_UpdateReturRanapPerTransNew
