   ...
   INCLUDE('ABREPORT.INC'),ONCE
                     ...
                     MAP
                       INCLUDE('APC6N118.INC'),ONCE        !Local module procedure declarations
                     END


PrintTransRawatJalan1 PROCEDURE                            ! Generated from procedure template - Report
...
report               REPORT,AT(10,41,80,1996),PAPER(PAPER:USER,8250,20000),PRE(RPT),FONT('Arial',10,,),MM
                       HEADER,AT(10,7,80,34),FONT('Times New Roman',8,,FONT:regular)
                         ...
                         STRING(@N010_),AT(14,5,17,4),USE(APH:Nomor_mr),FONT('Times New Roman',8,COLOR:Black,FONT:regular)
                         STRING('--'),AT(32,5,2,4),USE(?String30),TRN,FONT('Times New Roman',8,COLOR:Black,)
                         STRING(@s10),AT(36,5,17,4),USE(APH:Asal),FONT('Times New Roman',8,,FONT:regular)
                         STRING('No. RM :'),AT(2,5,12,4),USE(?String20),TRN,FONT('Times New Roman',8,COLOR:Black,FONT:regular)
                         STRING(@D8),AT(26,15,15,4),USE(APH:Tanggal),FONT('Times New Roman',8,,FONT:regular)
                         STRING(@t06),AT(42,15,13,4),USE(APH:Jam),FONT('Arial',8,,FONT:regular)
                         ...
                         BOX,AT(1,24,59,10),COLOR(COLOR:Black)
                         LINE,AT(1,29,59,0),USE(?Line11),COLOR(COLOR:Black)
                         LINE,AT(18,29,0,5),COLOR(COLOR:Black)
                         LINE,AT(18,24,0,5),USE(?Line5),COLOR(COLOR:Black)
                         LINE,AT(38,29,0,5),COLOR(COLOR:Black)
                         STRING('Diskon'),AT(40,29,9,4),USE(?String29),TRN,FONT('Times New Roman',8,COLOR:Black,)
                         STRING('Nama Barang'),AT(19,24,17,4),USE(?String10),TRN,FONT('Times New Roman',8,COLOR:Black,)
                         STRING(@s5),AT(47,1,9,4),USE(GL_entryapotik),TRN,FONT('Times New Roman',8,COLOR:Black,FONT:regular)
                         STRING('Jumlah'),AT(2,29,10,4),TRN,FONT('Times New Roman',8,COLOR:Black,)
                         STRING('Total'),AT(19,29,9,4),TRN,FONT('Times New Roman',8,COLOR:Black,)
                         STRING('Kode Barang'),AT(2,24,17,4),TRN,FONT('Times New Roman',8,COLOR:Black,)
                         STRING(@s15),AT(2,15,25,4),USE(APH:N0_tran),FONT('Times New Roman',8,COLOR:Black,FONT:regular)
...
detail1                  DETAIL,AT(,,184,8),USE(?detail1)
                           ...
                           STRING(@n-14.2),AT(16,3,19,4),USE(APD:Total),TRN,RIGHT,FONT('Times New Roman',8,,)
                           STRING(@n-15.2),AT(37,3,17,4),USE(APD:Diskon),TRN,RIGHT(2),FONT('Arial',8,,FONT:regular)
                           STRING(@n2),AT(54,3),USE(vl_disk),TRN,RIGHT(2),FONT('Times New Roman',8,,FONT:regular)
                           STRING('%'),AT(57,3,3,4),USE(?String20:2),TRN,FONT('Times New Roman',8,,FONT:regular)
                         ...
                         END
                         FOOTER,AT(0,0,,31),FONT('Times New Roman',8,,FONT:regular)
                           STRING(@n-14.2),AT(16,1,19,4),SUM,RESET(break1),USE(APD:Total,,?APD:Total:2),TRN,RIGHT,FONT('Times New Roman',8,COLOR:Black,FONT:regular)
                           STRING('-'),AT(36,1,1,4),USE(?String17:2),TRN,FONT('Times New Roman',8,COLOR:Black,)
                           STRING(@n15.2),AT(37,1,17,4),SUM,RESET(break1),USE(APD:Diskon,,?APD:Diskon:2),TRN,RIGHT(2),FONT('Arial',8,COLOR:Black,FONT:regular)
                           STRING('Petugas Apotik'),AT(36,11),USE(?String27),TRN,FONT('Times New Roman',8,COLOR:Black,)
                           STRING('2. Akuntansi'),AT(1,4),USE(?String25),TRN,FONT('Times New Roman',8,COLOR:Black,)
                           STRING(@s10),AT(37,19,17,3),USE(Glo:USER_ID),TRN,CENTER,FONT('Times New Roman',8,COLOR:Black,)
                           STRING(@s100),AT(1,23,61,4),USE(JKon:NAMA_KTR),TRN,FONT('Times New Roman',8,,)
                           STRING('Harga barang sudah termasuk PPN'),AT(1,27),USE(?String38),TRN
                           STRING('(.{26})'),AT(34,20),USE(?String28:3),TRN,FONT('Times New Roman',8,COLOR:Black,)
                           STRING('3. Arsip'),AT(1,7),USE(?String26),TRN,FONT('Times New Roman',8,COLOR:Black,)
                           STRING('Total : Rp.'),AT(17,6,16,4),USE(?String17),TRN,FONT('Times New Roman',8,COLOR:Black,)
                           STRING(@n-14.2),AT(32,6,20,4),USE(APH:Biaya),TRN,RIGHT,FONT('Times New Roman',8,COLOR:Black,FONT:regular)
                           STRING('1. Ybs.'),AT(1,1),USE(?String24),TRN,FONT('Times New Roman',8,COLOR:Black,)
...
ReturnValue          BYTE,AUTO
  ...
  CODE
  GlobalErrors.SetProcedureName('PrintTransRawatJalan1')
  ...
  INIMgr.Fetch('PrintTransRawatJalan1',ProgressWindow)     ! Restore window settings from non-volatile store
...
ReturnValue          BYTE,AUTO
  ...
  END
  IF SELF.Opened
    INIMgr.Update('PrintTransRawatJalan1',ProgressWindow)  ! Save window data to non-volatile store
  END
  ProgressMgr.Kill()
