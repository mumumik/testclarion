   ...
   INCLUDE('ABREPORT.INC'),ONCE
                     ...
                     MAP
                       INCLUDE('APC6N121.INC'),ONCE        !Local module procedure declarations
                     END


PrintReturRawatInap1 PROCEDURE                             ! Generated from procedure template - Report
...
report               REPORT,AT(10,37,80,2000),PAPER(PAPER:USER,8250,20000),PRE(RPT),FONT('Arial',10,,),MM
                       HEADER,AT(10,7,80,28)
                         ...
                         STRING(@t04),AT(41,13),USE(vl_jam),TRN,FONT('Arial',8,,FONT:regular)
                         ...
                         STRING(@s5),AT(51,1,9,4),USE(GL_entryapotik),TRN,FONT('Times New Roman',8,,FONT:regular)
...
detail1                  DETAIL,AT(,,184,8),USE(?detail1)
                           ...
                           STRING(@n-15.2),AT(36,3,17,5),USE(APD:Diskon),RIGHT,FONT('Arial',8,,FONT:regular)
                           STRING(@n2),AT(54,3,5,5),USE(vl_disk),TRN,RIGHT(2),FONT('Times New Roman',8,,FONT:regular)
                           STRING('%'),AT(57,3,5,5),USE(?String20:2),TRN,FONT('Times New Roman',8,,FONT:regular)
                         ...
                         END
                         FOOTER,AT(0,0,,29)
                           STRING(@n-14.2),AT(16,1,19,4),SUM,RESET(break1),USE(APD:Total,,?APD:Total:2),TRN,RIGHT,FONT('Times New Roman',8,,FONT:regular)
                           STRING('-'),AT(35,1,1,4),USE(?String17:2),TRN,FONT('Times New Roman',8,,)
                           STRING(@n-15.2),AT(36,1,17,4),SUM,RESET(break1),USE(APD:Diskon,,?APD:Diskon:2),TRN,RIGHT,FONT('Arial',8,,FONT:regular)
                           STRING('Petugas Apotik'),AT(8,11),USE(?String27),TRN,FONT('Times New Roman',8,,)
                           ...
                           STRING('(.{26})'),AT(30,20),USE(?String28:2),TRN,FONT('Times New Roman',8,,)
                           STRING(@s100),AT(1,24,52,4),USE(JKon:NAMA_KTR),FONT('Arial',8,,FONT:regular)
                           STRING(@s10),AT(9,19,17,3),USE(Glo:USER_ID),TRN,CENTER,FONT('Times New Roman',8,,)
                           STRING('(.{26})'),AT(6,20),USE(?String28:3),TRN,FONT('Times New Roman',8,,)
                           ...
                           STRING(@n-14.2),AT(32,6,20,4),USE(APH:Biaya),TRN,RIGHT,FONT('Times New Roman',8,,FONT:regular)
                           STRING('1. Perawat'),AT(1,1),USE(?String24),TRN,FONT('Times New Roman',8,,)
                           LINE,AT(0,0,122,0),USE(?Line9),COLOR(COLOR:Black)
                           LINE,AT(17,5,37,0),USE(?Line6),COLOR(COLOR:Black)
                           STRING('='),AT(55,3,1,4),USE(?String17:3),TRN,FONT('Times New Roman',8,,)
                         END
                       END
                     END
...
ReturnValue          BYTE,AUTO
  ...
  CODE
  GlobalErrors.SetProcedureName('PrintReturRawatInap1')
  ...
  INIMgr.Fetch('PrintReturRawatInap1',ProgressWindow)      ! Restore window settings from non-volatile store
...
ReturnValue          BYTE,AUTO
  ...
  END
  IF SELF.Opened
    INIMgr.Update('PrintReturRawatInap1',ProgressWindow)   ! Save window data to non-volatile store
  END
  ProgressMgr.Kill()
