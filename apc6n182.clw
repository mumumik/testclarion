   ...
   INCLUDE('ABWINDOW.INC'),ONCE
                     ...
                     MAP
                       INCLUDE('APC6N182.INC'),ONCE        !Local module procedure declarations
...
Trig_UpdateRawatInapCopyKeBpjs PROCEDURE                   ! Generated from procedure template - Window
...
QuickWindow          WINDOW('Transaksi Instalasi Farmasi -Rawat Inap'),AT(,,531,296),FONT('Arial',8,,),CENTER,IMM,HLP('UpdateAPHTRANS'),ALRT(EscKey),TIMER(100),GRAY,MDI
                       ...
                       BUTTON('F2'),AT(150,3,20,13),USE(?Button8),HIDE
...
ThisWindow.Ask PROCEDURE
  ...
  APHB:Nomor_mr = glo:bpjs_no_mr
  !Cek Pasien
  JPas:Nama = glo:bpjs_nama_pasien
  JPas:Alamat = glo:bpjs_alamat_pasien
  aphb:tanggal = glo:bpjs_tanggal
  aphb:biaya = glo:bpjs_biaya
  aphb:nonota = glo:bpjs_nonota
  aphb:jam = glo:bpjs_jam
  aphb:kode_apotik = glo:bpjs_kode_apotik
  aphb:asal = glo:bpjs_asal
  aphb:user = glo:bpjs_user
  aphb:cara_bayar = glo:bpjs_cara_bayar
  aphb:kontrak = glo:bpjs_kontrak
  aphb:dokter = glo:bpjs_dokter
  APHB:Urut = glo:bpjs_urut
  APHB:NoPaket = glo:bpjs_noPaket
  do isi_nomor
  apdtrans{prop:sql}='select * from dba.apdtrans where n0_tran='''&format(clip(glo:bpjs_no_tran),@s12)&''''
  apdtrans{prop:sql}='select * from dba.apdtrans where n0_tran='''&format(clip(glo:bpjs_no_tran),@s12)&''''
  loop
     ...
     GSTO:Kode_Apotik = glo:bpjs_kode_apotik
...
ReturnValue          BYTE,AUTO
  ...
  CODE
  GlobalErrors.SetProcedureName('Trig_UpdateRawatInapCopyKeBpjs')
  ...
  INIMgr.Fetch('Trig_UpdateRawatInapCopyKeBpjs',QuickWindow) ! Restore window settings from non-volatile store
...
ReturnValue          BYTE,AUTO
  ...
  END
  IF SELF.Opened
    INIMgr.Update('Trig_UpdateRawatInapCopyKeBpjs',QuickWindow) ! Save window data to non-volatile store
  END
