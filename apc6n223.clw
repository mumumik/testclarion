   ...
   INCLUDE('ABWINDOW.INC'),ONCE
                     ...
                     MAP
                       INCLUDE('APC6N223.INC'),ONCE        !Local module procedure declarations
                       INCLUDE('APC6N006.INC'),ONCE        !Req'd for module callout resolution
                       INCLUDE('APC6N196.INC'),ONCE        !Req'd for module callout resolution
                     END


Trig_UpdateReturRanapNewNonBilling PROCEDURE               ! Generated from procedure template - Window
...
QuickWindow          WINDOW('Pengembalian obat Rawat Inap ke Instalasi Farmasi'),AT(,,456,221),FONT('Times New Roman',8,COLOR:Black,),CENTER,IMM,HLP('UpdateAPHTRANS'),ALRT(EscKey),TIMER(100),GRAY,RESIZE,MDI
   ...
   'M~N 0 tran~C(0)@s15@44D(2)|M~Harga Dasar~C(0)@n11.2@64D(2)|M~total dtg~C(0)@N-16' &|
...
ReturnValue          BYTE,AUTO
  ...
  CODE
  GlobalErrors.SetProcedureName('Trig_UpdateReturRanapNewNonBilling')
  ...
  INIMgr.Fetch('Trig_UpdateReturRanapNewNonBilling',QuickWindow) ! Restore window settings from non-volatile store
...
ReturnValue          BYTE,AUTO
  ...
  END
  IF SELF.Opened
    INIMgr.Update('Trig_UpdateReturRanapNewNonBilling',QuickWindow) ! Save window data to non-volatile store
  END
...
ThisWindow.Reset PROCEDURE(BYTE Force=0)
  ...
  APH:Harga_Dasar = GLO:HARGA_DASAR_INAP_RETUR
...
ReturnValue          BYTE,AUTO
  ...
  ELSE
    ...
    Trig_UpdateReturRawatInapPerMRDetil
...
GLO:HARGA_DASAR_INAP_RETUR:Sum REAL                        ! Sum variable for browse totals
loc:total_dtg:Sum    REAL                                  ! Sum variable for browse totals
  ...
  LOOP
    ...
    END
    IF (APD:ktt=0)
      GLO:HARGA_DASAR_INAP_RETUR:Sum += APD:Harga_Dasar * APD:Jumlah
    END
    IF (APD:ktt=0)
  ...
  GLO:HARGA_DASAR_INAP_RETUR = GLO:HARGA_DASAR_INAP_RETUR:Sum
