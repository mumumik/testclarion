   ...
   INCLUDE('ABWINDOW.INC'),ONCE
                     ...
                     MAP
                       INCLUDE('APC6N104.INC'),ONCE        !Local module procedure declarations
                       INCLUDE('APC6N049.INC'),ONCE        !Req'd for module callout resolution
                     END


BrowseObatPerPasienPerTanggal PROCEDURE                    ! Generated from procedure template - Window
   ...
   '@n-15.2@60R|M~No Billing~L@s15@'),FROM(Queue:Browse:2)
                       SHEET,AT(4,4,503,121),USE(?CurrentTab)
                         TAB('Nomor MR'),USE(?Tab1)
                           ...
                           ENTRY(@n-14),AT(52,110,77,10),USE(VAP:Nomor_MR),HLP('Nama mendahului gelar'),MSG('Nama pasien'),REQ
                         END
                         TAB('Nama'),USE(?Tab2)
...
ReturnValue          BYTE,AUTO
  ...
  CODE
  GlobalErrors.SetProcedureName('BrowseObatPerPasienPerTanggal')
  ...
  INIMgr.Fetch('BrowseObatPerPasienPerTanggal',QuickWindow) ! Restore window settings from non-volatile store
...
ReturnValue          BYTE,AUTO
  ...
  END
  IF SELF.Opened
    INIMgr.Update('BrowseObatPerPasienPerTanggal',QuickWindow) ! Save window data to non-volatile store
  END
