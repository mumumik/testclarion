   ...
   INCLUDE('ABREPORT.INC'),ONCE
                     ...
                     MAP
                       INCLUDE('APC6N160.INC'),ONCE        !Local module procedure declarations
                       INCLUDE('APC6N004.INC'),ONCE        !Req'd for module callout resolution
                     END


PrintTransRawatJalanA5 PROCEDURE                           ! Generated from procedure template - Report
...
detail1                  DETAIL,AT(,,193,4),USE(?detail1)
                         ...
                         END
                         FOOTER,AT(0,0,,8),FONT('Times New Roman',8,,FONT:regular)
                           ...
                           STRING('Total : Rp.'),AT(79,0,16,4),USE(?String17),TRN,FONT('Times New Roman',8,COLOR:Black,)
                           LINE,AT(1,0,189,0),USE(?Line9),COLOR(COLOR:Black)
                           STRING(@N-15.2),AT(108,0,,4),USE(vl_total),TRN,RIGHT,FONT('Times New Roman',8,,)
                           STRING('Terbilang : '),AT(79,3),USE(?String41),TRN,FONT('Times New Roman',8,,,CHARSET:ANSI)
                           STRING(@s100),AT(93,3),USE(vl_totalhuruf),TRN,FONT('Times New Roman',8,,,CHARSET:ANSI)
                         END
                       END
                     END
...
ReturnValue          BYTE,AUTO
  ...
  CODE
  GlobalErrors.SetProcedureName('PrintTransRawatJalanA5')
  ...
  INIMgr.Fetch('PrintTransRawatJalanA5',ProgressWindow)    ! Restore window settings from non-volatile store
...
ReturnValue          BYTE,AUTO
  ...
  END
  IF SELF.Opened
    INIMgr.Update('PrintTransRawatJalanA5',ProgressWindow) ! Save window data to non-volatile store
  END
  ProgressMgr.Kill()
...
SkipDetails BYTE
  ...
  !vl_total=round(APD:Total,,?APD:Total:2,1)
