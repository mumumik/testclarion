   ...
   INCLUDE('ABWINDOW.INC'),ONCE
                     ...
                     MAP
                       INCLUDE('APC6N077.INC'),ONCE        !Local module procedure declarations
                       INCLUDE('APC6N058.INC'),ONCE        !Req'd for module callout resolution
                     END


BrowseBatalTidakUpdateBilling PROCEDURE                    ! Generated from procedure template - Window
...
Kode_brg             STRING(10),DIM(20)                    !Kode Barang
Jumlah               REAL,DIM(20)                          !Jumlah
J_potong             REAL,DIM(20)                          !Jumlah yang dipotong ke tabel obat
Total                REAL,DIM(20)                          !Total harga barang / item
Camp                 ULONG,DIM(20)                         !
Harga_Dasar          REAL,DIM(20)                          !Harga Dasar
Kode_brg_apd         STRING(10),DIM(20)                    !Kode Barang
Jumlah_apd           REAL,DIM(20)                          !Jumlah
Total_apd            REAL,DIM(20)                          !Total harga barang / item
Camp_apd             ULONG,DIM(20)                         !
Harga_Dasar_apd      REAL,DIM(20)                          !Harga Dasar
DISKON_APD           REAL,DIM(20)                          !
...
QuickWindow          WINDOW('Pembatalan Transaksi Instalasi Farmasi- Rawat Jalan'),AT(,,457,234),FONT('Arial',8,,),CENTER,IMM,HLP('Tran_Poliklinik'),SYSTEM,GRAY,MDI
                       LIST,AT(10,20,435,62),USE(?Browse:1),IMM,HVSCROLL,MSG('Browsing Records'),VCR,FORMAT('58L|FM~Nomor RM~C@N010_@51R(1)|M~Tanggal~C(0)@d06@54R(1)|M~Biaya~C(0)@n-15.2@64L' &|
   '|M~No. Transaksi~C@s15@25L|M~ Apotik~C@s5@40L|M~No Nota~C@s10@20L|M~Lunas~@s5@32' &|
   'L|M~Poliklinik~C@s1@44L|M~Asal~@s10@16L|M~User~@s4@12L|M~Batal~@n3@'),FROM(Queue:Browse:1)
                       LIST,AT(5,127,443,79),USE(?List),IMM,HVSCROLL,MSG('Browsing Records'),FORMAT('52L|FM~Kode Barang~C@s10@147L|FM~Nama Obat~C@s40@68L|FM~Keterangan~C@s50@62R(14)' &|
   '|M~Jumlah~C(0)@n-12.2@63D(14)|M~Total~C(0)@n-15.2@60D(14)|M~Diskon~C(0)@n-15.2@6' &|
   '0L~Camp~C@n2@'),FROM(Queue:Browse)
                       BUTTON('&Batalkan'),AT(368,85,78,26),USE(?Button9),LEFT,FONT('Arial',8,COLOR:Blue,FONT:bold),ICON(ICON:Cut)
                       ...
                       SHEET,AT(5,4,445,111),USE(?CurrentTab)
                         TAB('No. Nota [F3]'),USE(?Tab:3),KEY(F3Key)
                           BUTTON('Cetak Detil'),AT(15,85,69,26),USE(?Button8),LEFT,ICON(ICON:Print)
                           PROMPT('No Transaksi :'),AT(102,92),USE(?APH:N0_tran:Prompt)
                           ENTRY(@s15),AT(152,92,60,10),USE(APH:N0_tran),MSG('nomor transaksi'),TIP('nomor transaksi')
                         END
                       END
                       BUTTON('&Tutup'),AT(365,210,83,22),USE(?Close),LEFT,ICON(ICON:Cross)
...
ReturnValue          BYTE,AUTO
  ...
  CODE
  GlobalErrors.SetProcedureName('BrowseBatalTidakUpdateBilling')
  ...
  INIMgr.Fetch('BrowseBatalTidakUpdateBilling',QuickWindow) ! Restore window settings from non-volatile store
...
ReturnValue          BYTE,AUTO
  ...
  END
  IF SELF.Opened
    INIMgr.Update('BrowseBatalTidakUpdateBilling',QuickWindow) ! Save window data to non-volatile store
  END
