   ...
   INCLUDE('ABREPORT.INC'),ONCE
                     ...
                     MAP
                       INCLUDE('APC6N123.INC'),ONCE        !Local module procedure declarations
                     END


PrintTransRawatInap PROCEDURE                              ! Generated from procedure template - Report
...
report               REPORT,AT(10,39,80,1998),PAPER(PAPER:USER,8250,30000),PRE(RPT),FONT('Arial',10,,),MM
                       HEADER,AT(10,7,80,32)
                         ...
                         STRING(@s20),AT(34,5,21,5),USE(ITbr:NAMA_RUANG),TRN,FONT(,8,,)
...
detail1                  DETAIL,AT(,,184,8),USE(?detail1)
                         ...
                         FOOTER,AT(0,0,,29)
                           ...
                           STRING(@n-14.2),AT(35,6,20,4),USE(APH:Biaya),TRN,RIGHT,FONT('Times New Roman',8,,FONT:regular)
...
ReturnValue          BYTE,AUTO
  ...
  CODE
  GlobalErrors.SetProcedureName('PrintTransRawatInap')
  ...
  INIMgr.Fetch('PrintTransRawatInap',ProgressWindow)       ! Restore window settings from non-volatile store
...
ReturnValue          BYTE,AUTO
  ...
  END
  IF SELF.Opened
    INIMgr.Update('PrintTransRawatInap',ProgressWindow)    ! Save window data to non-volatile store
  END
  ProgressMgr.Kill()
