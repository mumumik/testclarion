   ...
   INCLUDE('ABWINDOW.INC'),ONCE
                     ...
                     MAP
                       INCLUDE('APC6N225.INC'),ONCE        !Local module procedure declarations
                       ...
                       INCLUDE('APC6N226.INC'),ONCE        !Req'd for module callout resolution
                     END


Trig_BrowseReturRanapNewPerNotaNonBilling PROCEDURE        ! Generated from procedure template - Window
...
ViewPosition           STRING(1024)                   !Entry's view position
                     END
QuickWindow          WINDOW('Pengembalian Obat Rawat Inap'),AT(,,457,205),FONT('Times New Roman',8,COLOR:Black,),IMM,HLP('Tran_Poliklinik'),SYSTEM,GRAY,RESIZE,MDI
                       LIST,AT(8,20,436,53),USE(?Browse:1),IMM,HVSCROLL,MSG('Browsing Records'),VCR,FORMAT('64L|M~No. Transaksi~C@s15@58R(1)|M~Tanggal~C(0)@D8@64R(1)|M~Biaya~C(0)@n-15.2@32' &|
   'L|M~Lunas~@s5@30L|M~User~@s4@121L|M~cara bayar~@n1@20L|M~Kode Apotik~@s5@28R|M~U' &|
   'rut~L@n-7@12R|M~Batal~L@n3@60R|M~No Transaksi Asal~L@s15@'),FROM(Queue:Browse:1)
                       LIST,AT(5,109,443,68),USE(?List),IMM,HVSCROLL,MSG('Browsing Records'),FORMAT('52L|FM~Kode Barang~C@s10@121L|FM~Nama Obat~C@s40@87L|FM~Keterangan~C@s50@17L|FM~' &|
   'KTT~C@n3@53R(2)|M~Jumlah~C(0)@n-12.2@63R(2)|M~Total~C(0)@n-15.2@60R(2)|M~Diskon~' &|
   'C(0)@n-15.2@60L|M~Camp~C@n2@60L|M~N 0 tran~C@s15@64D|M~total dtg~C@N-16.2@'),FROM(Queue:Browse)
                       BUTTON('T&ransaksi (F4)'),AT(367,77,72,26),USE(?Insert:3),LEFT,FONT('Times New Roman',8,COLOR:Lime,FONT:bold),MSG('Transaksi Pengembalian Obat'),TIP('Transaksi Pengembalian Obat'),KEY(F4Key),ICON(ICON:Open)
                       ...
                       SHEET,AT(4,4,445,102),USE(?CurrentTab)
                         TAB('No. Nota'),USE(?Tab:3)
                           BUTTON('Cetak &Detail'),AT(12,77,61,26),USE(?Print),LEFT,FONT('Times New Roman',8,,FONT:bold),MSG('Cetak SBBM'),TIP('Cetak SBBM'),ICON(ICON:Print)
                           BUTTON('Cetak &Nota'),AT(83,77,61,26),USE(?Print:2),LEFT,FONT('Times New Roman',8,COLOR:Blue,FONT:bold),MSG('Cetak Nota Transaksi'),TIP('Mencetak Nota Transaksi'),ICON(ICON:Print1)
                         END
                       END
                       BUTTON('&Tutup'),AT(357,183,87,20),USE(?Close),LEFT,FONT('Times New Roman',10,COLOR:Teal,FONT:bold+FONT:italic),ICON(ICON:Cross)
...
ReturnValue          BYTE,AUTO
  ...
  CODE
  GlobalErrors.SetProcedureName('Trig_BrowseReturRanapNewPerNotaNonBilling')
  ...
  Relate:APDTRANS.Open                                     ! File NomorUse used by this procedure, so make sure it's RelationManager is open
  Relate:JDBILLING.Open                                    ! File NomorUse used by this procedure, so make sure it's RelationManager is open
  Relate:JDDBILLING.Open                                   ! File NomorUse used by this procedure, so make sure it's RelationManager is open
  ...
  BRW1.SetFilter('((APH:N0_tran=glo::no_nota OR APH:N0_tran=GLO:NOBATAL) and APH:N0_tran<<>'''' and sub(APH:N0_tran,1,3)=''APO'' and APH:Ra_jal=0)') ! Apply filter expression to browse
  ...
  BRW1.AddField(APH:Urut,BRW1.Q.APH:Urut)                  ! Field APH:Urut is a hot field or requires assignment from browse
  ...
  INIMgr.Fetch('Trig_BrowseReturRanapNewPerNotaNonBilling',QuickWindow) ! Restore window settings from non-volatile store
...
ReturnValue          BYTE,AUTO
  ...
  END
  IF SELF.Opened
    INIMgr.Update('Trig_BrowseReturRanapNewPerNotaNonBilling',QuickWindow) ! Save window data to non-volatile store
  END
...
ReturnValue          BYTE,AUTO
  ...
  ELSE
    ...
    EXECUTE Number
      Trig_UpdateReturRanapPerTransNewNonBilling
...
Looped BYTE
  ...
  LOOP                                                     ! This method receive all EVENT:Accepted's
    ...
    OF ?Insert:3
      ...
      vg_notrans= APH:N0_tran
      vg_kodeapotik = APH:Kode_Apotik
      
      if APH:Biaya<0 or APH:NoTransaksiAsal<>'' then
          message('Tidak dapat dilanjutkan karena transaksi tersebut adalah transaksi retur')
          cycle
      elsif APH:Batal =1 then
          message('Transaksi tesebut sudah pernah dibatalkan, tidak dapat dilanjutkan')
          cycle
      else
      end
      !message(Glo::no_mr)
