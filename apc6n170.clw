   ...
   INCLUDE('ABREPORT.INC'),ONCE
                     ...
                     MAP
                       INCLUDE('APC6N170.INC'),ONCE        !Local module procedure declarations
                     END


PrintReturRawatInap1CDbpjs PROCEDURE                       ! Generated from procedure template - Report
...
report               REPORT,AT(10,37,80,2000),PAPER(PAPER:USER,8250,30000),PRE(RPT),FONT('Arial',10,,),MM
                       HEADER,AT(10,7,80,28)
                         ...
                         STRING(@D06),AT(26,13),USE(APHB:Tanggal),FONT('Times New Roman',8,,FONT:regular,CHARSET:ANSI)
                         STRING(@s15),AT(2,13,24,5),USE(APHB:N0_tran),FONT('Times New Roman',8,,FONT:regular,CHARSET:ANSI)
                         ...
                         STRING(@s10),AT(38,5,17,5),USE(APHB:Asal),FONT(,8,,,CHARSET:ANSI)
                         STRING(@N010_),AT(15,5),USE(APHB:Nomor_mr),FONT(,8,,,CHARSET:ANSI)
                       END
...
ReturnValue          BYTE,AUTO
  ...
  CODE
  GlobalErrors.SetProcedureName('PrintReturRawatInap1CDbpjs')
  ...
  INIMgr.Fetch('PrintReturRawatInap1CDbpjs',ProgressWindow) ! Restore window settings from non-volatile store
...
ReturnValue          BYTE,AUTO
  ...
  END
  IF SELF.Opened
    INIMgr.Update('PrintReturRawatInap1CDbpjs',ProgressWindow) ! Save window data to non-volatile store
  END
  ProgressMgr.Kill()
