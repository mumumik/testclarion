   ...
   INCLUDE('ABWINDOW.INC'),ONCE
                     ...
                     MAP
                       INCLUDE('APC6N205.INC'),ONCE        !Local module procedure declarations
                     END


cari_brg_lokal5 PROCEDURE                                  ! Generated from procedure template - Window
...
QuickWindow          WINDOW('Melihat Data Barang'),AT(,,417,190),FONT('Arial Narrow',10,,),IMM,HLP('cari_brg_lokal4'),SYSTEM,GRAY,MDI
                       LIST,AT(8,20,401,130),USE(?Browse:1),IMM,HVSCROLL,FONT(,12,,),MSG('Browsing Records'),FORMAT('48L(2)|M~Kode Barang~@s10@137L(2)|M~Nama Obat~@s40@138L(2)|M~Kandungan~@s50@54R(' &|
   '2)|M~Stok~L@n16.2@65R(2)|M~Harga Dasar~L@n15.2@24R(2)|M~Kelompok~L@n6@12R(2)|M~S' &|
   'tatus~L@n3@20R(2)|M~Kode Apotik~L@s5@12R(2)|M~Far Non Far~L@n3@'),FROM(Queue:Browse:1)
                       ...
                       SHEET,AT(3,4,411,182),USE(?CurrentTab)
                         TAB('Kode Barang [F2]'),USE(?Tab:2),KEY(F2Key)
                           ...
                           PROMPT('Kode Barang : '),AT(14,153),USE(?GBAR:Kode_brg:Prompt)
                         END
...
ReturnValue          BYTE,AUTO
  ...
  CODE
  GlobalErrors.SetProcedureName('cari_brg_lokal5')
  ...
  BRW1.SetFilter('(gbar:status=1 and gbar:farnonfar=0 and gsto:kode_apotik=glo:apotikfilter and gsto:saldo<<>0 and sub(gbar:kode_brg,1,7)=''_campur'')') ! Apply filter expression to browse
  ...
  INIMgr.Fetch('cari_brg_lokal5',QuickWindow)              ! Restore window settings from non-volatile store
...
ReturnValue          BYTE,AUTO
  ...
  END
  IF SELF.Opened
    INIMgr.Update('cari_brg_lokal5',QuickWindow)           ! Save window data to non-volatile store
  END
